...
namespace KotorTool_2._0.MainForm
{
    ...
    {
        public Form OpenFileFromCmdLine(MainAppState mainState)
        {
            /*
            int num1 = 0;
        ...
        private void HandleMdlFiles(KotorTreeNode node, TreeView treeView)
        {
            ...
            if (StringType.StrCmp(resTypeStr, "mdl", false) == 0)
            {
                if (ConfigOptions.Paths.ModelExportLocation == null || !Directory.Exists(ConfigOptions.Paths.ModelExportLocation))
                {
                    ...
                    {
                        if (!Directory.Exists(MainAppState.GameRootPath + "working\\Exported Models"))
                        {
                            Directory.CreateDirectory(MainAppState.GameRootPath + "working\\Exported Models");
                        }

                        frmPathManager.tbModuleExportPath.Text = MainAppState.GameRootPath + "working\\Exported Models";
                ...
                if (!frmMdlOpsSwitches.chkbMdlAlignDataOnly.Checked)
                {
                    ...
                    while (num2 <= int32)
                    {
                        ...
                        string str1 = MainAppState.GameRootPath + "mdlops.exe";
                        try
                        {
