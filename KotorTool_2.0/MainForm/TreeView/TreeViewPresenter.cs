...
namespace KotorTool_2._0.MainForm
{
    public class TreeViewPresenter
    {
        ...
        private MainAppState _mainState;
        ...
        public TreeViewPresenter(MainAppState mainState, TreeView treeView)
        {
        ...
        public void BuildTreeView(KotorTreeNode root, bool noProgressIndication = false)
        {
            ...
            int num3;
            byte[] numArray;
            FileStream fs = FStream.Generate(ConfigOptions.Paths.KeyFileLocation(0));
            int num2;
            ...
            binaryReader.ReadInt32();
            _mainState.BiffEntries[index1] = new ArrayList(capacity);
            int num4 = 0;
            int num5 = capacity - 1;
            int index2 = num4;
            while (index2 <= num5)
            {
                _mainState.BiffEntries[index1].Add(new BiffEntry(index2, fs, num2 + 12 * index2));
                _mainState.BiffEntryListArray[index1, index2] = new ArrayList();
                ++index2;
            }
            ...
            while (index3 <= num8)
            {
                ...
                _mainState.BiffEntryListArray[index1, keyEntry.ResourceId >> 20].Add(keyEntry);
            ...
            while (index4 <= num11)
            {
                ...
                if (index4 != 18 || buildModelsBifNode)
                {
                    ...
                    int num12 = 0;
                    int num13 = _mainState.BiffEntryListArray[index1, index4].Count - 1;
                    int index5 = num12;
                    while (index5 <= num13)
                    {
        ...
        public void OrganizeNodesByResType(KotorTreeNode collection, KotorTreeNode node)
        {
            ...
            KotorTreeNode kotorTreeNode = new KotorTreeNode(ResourceIdentification.ParentDescForResourceType(node.ResType)) {ResType = node.ResType, Tag = "NodeCategory"};
        ...
        public void CreateTemplateTagsHashFiles()
        {
            ...
            if (_mainState.HasKotor1 && !File.Exists("K1TemplateTags.bfd"))
            {
                ...
                foreach (KeyEntry keyEntry in _mainState.BiffEntryListArray[0, 23])
                {
                    if (!hashtable1.ContainsKey(Convert.ToInt32(keyEntry.ResourceType))) continue;

                    string gffTag = GffObject.GetGffTag(BiffFunctions.K1TemplatesBif().GetBiffResource(keyEntry.ResourceId - keyEntry.ResourceId >> 20 << 20).Data);
                    hashtable2.Add(keyEntry.ResourceRef + "." + ResourceIdentification.GetResourceTypeForId(keyEntry.ResourceType), gffTag);
            ...
            foreach (KeyEntry keyEntry in _mainState.BiffEntryListArray[1, 2])
            {
                if (hashtable1.ContainsKey(Convert.ToInt32(keyEntry.ResourceType)))
                {
                    string gffTag = GffObject.GetGffTag(BiffFunctions.K2TemplatesBif().GetBiffResource(keyEntry.ResourceId - keyEntry.ResourceId >> 20 << 20).Data);
                    hashtable3.Add(keyEntry.ResourceRef + "." + ResourceIdentification.GetResourceTypeForId(keyEntry.ResourceType), gffTag);
                    if (StringType.StrCmp(gffTag, "", false) == 0)
                    {
                        Console.WriteLine("-----------------------------");
                    }
        ...
        private void MiBasicHelpClick(object sender, EventArgs e)
        {
            try
            {
                Process.Start(MainAppState.GameRootPath + "KT_Help.chm");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Separate this out betweeen collections, iterations, and execution of form
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void MiViewGffFileOnClick(object sender, EventArgs e)
        {
            ...
            while (index <= num2)
            {
                ...
                clsGff.Parse(FStream.Generate(strArray[index]));
