...
namespace KotorTool_2._0.Models.CLS
{
    public class ClsDialogTlk : IDialogTlk
    {
        ...
        public ClsDialogTlk(string path, bool memoryResident = false)
        {
            ...
            if (!memoryResident) return;
            _fs.Seek(0L, SeekOrigin.Begin);
            _bytes = new byte[(int) (_fs.Length - 1L) + 1];
            _bytes = _rdr.ReadBytes((int) _fs.Length);
            _rdr.Close();
        ...
        {
            if (stringRef > checked(StringCount - 1)) return "";
            _rdr.BaseStream.Seek(checked(20 + stringRef * 40), SeekOrigin.Begin);
            if ((uint) (_rdr.ReadInt32() & 1) <= 0U) return "";
        ...
        {
            if (stringRef > StringCount - 1)
            {
                return string.Empty;
            }

            _rdr.BaseStream.Seek(0L, SeekOrigin.Begin);
            _rdr.BaseStream.Seek(20 + stringRef * 40, SeekOrigin.Begin);
        ...
        {
            if (stringRef > checked(StringCount - 1)) return 0.0f;
            _fs.Seek(0L, SeekOrigin.Begin);
            _fs.Seek(checked(20 + stringRef * 40), SeekOrigin.Begin);
            if ((uint) (_rdr.ReadInt32() & 4) <= 0U) return 0.0f;
        ...
        {
            if (stringRef > StringCount - 1) return null;
            int num1 = checked(20 + stringRef * 40);
            _rdr.BaseStream.Seek(num1, SeekOrigin.Begin);
            DialogTlkStrDataElement tlkStrDataElement = new DialogTlkStrDataElement {Flags = _rdr.ReadInt32()};
            if (stringRef == 31483) ;
            ...
            {
                _rdr.BaseStream.Seek(num1 + 36, SeekOrigin.Begin);
            ...
            {
                StringBuilder stringBuilder = new StringBuilder();
                _rdr.BaseStream.Seek(num1 + 28, SeekOrigin.Begin);
                int num2 = _rdr.ReadInt32();
                int count = _rdr.ReadInt32();
                _rdr.BaseStream.Seek(_stringEntriesOffset + num2, SeekOrigin.Begin);
                stringBuilder.Append(_rdr.ReadChars(count));
        ...
        public void WriteListToFile(string path)
        {
            ...
            int num1 = 20 + _talkEntryList.Count * 40;
            int offset = num1;
            ...
            int num2 = 0;
            int num3 = _talkEntryList.Count - 1;
            int index = num2;
            while (index <= num3)
            {
                binaryWriter.Seek(20 + index * 40, SeekOrigin.Begin);
                ...
                if ((uint) (talkEntry.Flags & 1) > 0U)
                    binaryWriter.Write(offset - num1);
                else
                    binaryWriter.Write(Convert.ToInt32(0));
