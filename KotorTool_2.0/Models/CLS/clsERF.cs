...
namespace KotorTool_2._0.Models.CLS
{
    public class ClsErf : IErf
    {
        ...
        public ClsErf(string outputFilePath, string fileType, uint descriptionStrRef,
            ErfLocalizedString[] erfLocalizedStringList, string[] inputFileList)
        {
            ...
            fileStream.Seek(checked(num1 + 160), SeekOrigin.Begin);
            int num8 = 0;
            int num9 = checked(inputFileList.Length - 1);
            int index3 = num8;
            while (index3 <= num9)
            {
                int index1 = 0;
                ...
                {
                    int num3 = checked(withoutExtension.Length - 1);
                    int index2 = num2;
                    while (index2 <= num3)
                    {
        ...
        public ClsErf(FileStream fs)
        {
            ...
            while (index <= num2)
            {
                ...
                int num3 = 0;
                while (_keyData[checked(num3 + index * 24)] != 0)
                {
                    ++num3;
                    if (num3 > 15) break;
                }
                ...
                int resourceId = checked((int) Math.Round(_keyData[checked(index * 24 + 16)] +
                ...
                int offset = checked((int) Math.Round(_resInfoData[checked(index * 8)] +
                                                      _resInfoData[checked(index * 8 + 1)] * 256.0 +
                                                      _resInfoData[checked(index * 8 + 2)] * 65536.0 +
                                                      _resInfoData[checked(index * 8 + 3)] * 16777216.0));
                int length = checked((int) Math.Round(_resInfoData[checked(index * 8 + 4)] +
