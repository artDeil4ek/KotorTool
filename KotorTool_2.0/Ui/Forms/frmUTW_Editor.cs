...
// Type: KotorTool_2.frmUTW_Editor
...
namespace KotorTool_2._0.Ui.Forms
{
  public class frmUTW_Editor : frmParent
  {
    ...
    {
      Load += frmUTW_Editor_Load;
      Closing += frmUTW_Editor_Closing;
      ComboBoxSettingError = false;
      m_bSaveGameMode = false;
      InitializeComponent();
    }

    public frmUTW_Editor(ClsUtw UTWClass, int KotorVerIndex, bool IsDirectEdit = false, string EditingPath = "")
      : this()
    {
      if (IsDirectEdit)
      {
      ...
      m_defaultTitleText = "Waypoint Editor - KotOR ";
    ...
    private void btnSave_Click(object sender, EventArgs e)
    {
      ...
      if (!m_bSaveGameMode)
      {
        m_EditingFilePath = StringType.FromObject(FileUtils.GetFilePath("save", ConfigOptions.Paths.DefaultSaveLocation, Path.GetFileNameWithoutExtension(m_EditingFilePath) + ".utw", "", ""));
        if (StringType.StrCmp(m_EditingFilePath, "", false) == 0)
          return;
      }
      UTW.WriteFile(m_EditingFilePath, "UTW");
      if (!m_bSaveGameMode)
        return;
      Close();
    }
    ...
    private void frmUTW_Editor_Load(object sender, EventArgs e)
    {
      PositionWindow(GetType().Name);
    }

    private void frmUTW_Editor_Closing(object sender, CancelEventArgs e)
    {
