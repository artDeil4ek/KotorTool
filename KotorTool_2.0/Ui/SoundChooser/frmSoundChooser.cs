...
// Type: KotorTool_2.frmSoundChooser
...
namespace KotorTool_2._0.Ui.SoundChooser
{
  public class frmSoundChooser : Form
  {
    ...
    [AccessedThroughProperty("tbFilter")]
    ...
    internal virtual CheckBox chkbLookSoundsBif
    {
      ...
      [MethodImpl(MethodImplOptions.Synchronized)] set
      {
        if (_chkbLookSoundsBif != null)
          _chkbLookSoundsBif.CheckedChanged -= LookForSoundsChkBxsChanged;
    ...
    internal virtual CheckBox chkbLookStreamSounds
    {
      ...
      [MethodImpl(MethodImplOptions.Synchronized)] set
      {
        if (_chkbLookStreamSounds != null)
          _chkbLookStreamSounds.CheckedChanged -= LookForSoundsChkBxsChanged;
    ...
    internal virtual TextBox tbFilter
    {
      ...
      {
        if (_tbFilter != null)
          _tbFilter.TextChanged -= tbFilter_TextChanged;
        _tbFilter = value;
        if (_tbFilter == null)
          return;
        _tbFilter.TextChanged += tbFilter_TextChanged;
    ...
    internal virtual ComboBox cmbxFilter
    {
      ...
      {
        if (_cmbxFilter != null)
          _cmbxFilter.SelectedIndexChanged -= cmbxFilter_SelectedIndexChanged;
        _cmbxFilter = value;
        if (_cmbxFilter == null)
          return;
        _cmbxFilter.SelectedIndexChanged += cmbxFilter_SelectedIndexChanged;
    ...
    {
      if (!((frmUTS_Editor) Owner).PlaySound(StringType.FromObject(ObjectType.StrCatObj(lbSounds.SelectedItem, ".wav"))))
        return;
      btnPlaySound.Enabled = false;
    ...
    {
      btnPlaySound.Enabled = !((frmUTS_Editor) Owner).IsSoundPlaying() && lbSounds.SelectedIndices.Count > 0;
    }
    ...
    private void AddSoundFileHashesToLB()
    {
      ...
      if (_soundBifFilesHash != null && chkbLookSoundsBif.Checked)
      {
        ...
        {
          DictionaryEntry dictionaryEntry = (DictionaryEntry) (obj ?? Activator.CreateInstance(typeof (DictionaryEntry)));
          if ((StringType.StrCmp(selectedItem._filter, "", false) == 0 || StringType.StrCmp(selectedItem._filter, "", false) != 0 && dictionaryEntry.Key.ToString().StartsWith(selectedItem._filter)) && dictionaryEntry.Key.ToString().IndexOf(tbFilter.Text) > -1)
      ...
      lbSounds.EndUpdate();
      this.lblSoundCount.Text = StringType.FromInteger(lbSounds.Items.Count) + " Sound";
      if (lbSounds.Items.Count <= 1 && lbSounds.Items.Count != 0)
        return;
      Label lblSoundCount = this.lblSoundCount;
      lblSoundCount.Text = lblSoundCount.Text + "s";
    }

    private void LookForSoundsChkBxsChanged(object sender, EventArgs e)
    {
      AddSoundFileHashesToLB();
    }
    ...
    private void frmSoundChooser_Load(object sender, EventArgs e)
    {
      ((frmUTS_Editor) Owner).StoppedPlaying += OwnerStoppedPlaying;
    }
